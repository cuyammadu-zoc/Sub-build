...
class NetworkLink:
    def __init__(self, target_ip, target_port):
        ...
        self.telemetry = {
            "mode": 0, "depth": 0.0, "target_depth": 0.5,
            "heading": 0.0, "pitch": 0.0, "temp": 18.5, "ax": 0.0, "ay": 0.0
        }

    def send_command(self, fl, fr, bl, br, mode=0, target_depth=0.5):
        packet = {
            "fl": fl, "fr": fr, "bl": bl, "br": br,
            "mode": mode, "target_depth": target_depth
        }
        try:
...
class HUD:
    ...
    def draw_motor_bar(self, x, y, label, pwm_value):
        ...
        normalized = (pwm_value - 1500) / 400.0
...
def main():
    ...
    pygame.display.set_caption("Submarine Ground Control HUD v2.1")
    ...
    mode = 0           # 0: MANUAL, 1: DEPTH_HOLD, 2: EMERGENCY_SURFACE
    target_depth = 1.0 # Target Depth (Meters)
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_1:
                    ...
                    target_depth = round(min(10.0, target_depth + 0.1), 1)
                elif event.key == pygame.K_DOWN:
                    target_depth = round(max(0.0, target_depth - 0.1), 1)
            elif event.type == pygame.JOYBUTTONDOWN:
                if event.button == 0:    # PS5 Cross (X) -> Manual Mode
                    mode = 0
                elif event.button == 2:  # PS5 Square -> Depth Hold Mode
                    mode = 1
                elif event.button == 3:  # PS5 Triangle -> Emergency Surface
                    mode = 2
                elif event.button == 11: # D-Pad Up -> Target Depth +0.1m
                    target_depth = round(min(10.0, target_depth + 0.1), 1)
                elif event.button == 12: # D-Pad Down -> Target Depth -0.1m
                    target_depth = round(max(0.0, target_depth - 0.1), 1)
        ...
        net.send_command(fl, fr, bl, br, mode=mode, target_depth=target_depth)
        ...
        mode_names = ["0: MANUAL", "1: DEPTH HOLD", "2: EMERGENCY SURFACE"]
        mode_str = mode_names[mode] if mode < len(mode_names) else "0: MANUAL"
        ...
        for idx, line in enumerate(telem_lines):
            color = HUD_CYAN if "[" in line else (HUD_GREEN if "HARDWARE" in line and joystick else TEXT_WHITE)
            if "MODE:" in line and mode == 1:
                color = HUD_GREEN
            elif "MODE:" in line and mode == 2:
                color = HUD_RED
            screen.blit(hud.font.render(line, True, color), (465, 75 + (idx * 21)))
